...
public class CompletableFutureTest06 {
    ...
    private static void searchPricesWithDiscountAsync(StoreServiceWithDiscount service) {
        ...
        var completableFutures = stores
                .stream()
                .map(s -> CompletableFuture
                        .supplyAsync(() -> service.getPriceSync(s)))
                .map(cf -> cf
                        .thenApply(Quote::newQuote))
                .map(cf -> cf
                        .thenCompose(quote -> CompletableFuture
                                .supplyAsync(() -> service
                                        .applyDiscount(quote))))
                .map(cf -> cf
                        .thenAccept(store -> System.out.printf("%s finished in %d ms%n", store, (System.currentTimeMillis() - start))))
